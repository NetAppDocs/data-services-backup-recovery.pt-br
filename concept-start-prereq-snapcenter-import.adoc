...
---
= SnapCenter pré-requisitos de importação para NetApp Backup and Recovery
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Antes de importar recursos do SnapCenter Plug-in for Microsoft SQL Server para o NetApp Backup and Recovery, certifique-se de que seu ambiente atenda a estes requisitos adicionais.
...
== Garanta os requisitos do host para o SnapCenter Plug-in

Para importar recursos do SnapCenter Plug-in for Microsoft SQL Server, certifique-se de que os requisitos de host para a instalação do SnapCenter Plug-in for Microsoft SQL Server sejam atendidos.
...
** `1`: As restrições remotas do UAC estão desativadas. As contas locais ignoram a filtragem de tokens e têm privilégios administrativos completos ao acessar remotamente.
