...
---
= Diferenças de política entre SnapCenter e NetApp Backup and Recovery
:hardbreaks:
:allow-uri-read: 
:icons: font
:imagesdir: ./media/
...
[role="lead"]
Use esta referência para entender as diferenças de política entre SnapCenter e NetApp Backup and Recovery que podem afetar os recursos e as políticas importadas.
...
NetApp Backup and Recovery usa os seguintes níveis de agendamento, que diferem ligeiramente:
...
NetApp Backup and Recovery ajusta os agendamentos do SnapCenter da seguinte forma:
...
* Se o agendamento do SnapCenter estiver definido para mais de 7 dias, o NetApp Backup and Recovery definirá o agendamento como mensal. Alguns snapshots serão ignorados durante o mês.
+
...
*Exemplo*: Se você tiver uma política SnapCenter de execução por hora com um intervalo de repetição a cada 5 horas, começando à 1h da manhã, NetApp Backup and Recovery (após a migração) definirá a programação para diária, com intervalos de 5 horas às 1h, 6h, 11h, 16h e 21h. Algumas horas serão puladas — após as 21h, o horário de repetição a cada 5 horas deveria ser 2h da manhã, mas sempre será 1h da manhã.
