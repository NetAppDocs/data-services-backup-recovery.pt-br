...
[role="lead"]
No NetApp Backup and Recovery, crie suas próprias políticas de backup KVM que determinam a frequência do backup, o horário em que o backup é realizado e o número de arquivos de backup que são retidos.
...
** *Configurações da VM*:
+
*** *Habilitar snapshot consistente com a VM e com o aplicativo*: habilite esta opção para criar snapshots consistentes com a VM e com o aplicativo. Isso requer que o KVM QEMU Guest Agent esteja em execução na VM. Se o Guest Agent não estiver em execução, os snapshots serão consistentes com falhas. Observe que habilitar esta opção pode aumentar o tempo de backup e usar mais armazenamento. Além disso, a RAM ativa da VM não é incluída nos snapshots consistentes.
