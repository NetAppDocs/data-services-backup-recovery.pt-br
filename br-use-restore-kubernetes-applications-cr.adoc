...
---
= Restaurar aplicativos Kubernetes com recursos personalizados
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Use recursos personalizados para restaurar seus aplicativos a partir de um snapshot ou backup. A restauração a partir de um snapshot existente geralmente é mais rápida quando você restaura o aplicativo no mesmo cluster.
...
* Você pode usar configurações avançadas para personalizar as operações de restauração. Para saber mais, consulte link:br-use-kubernetes-advanced-restore-settings.html["Configurações avançadas de restauração para Kubernetes em NetApp Backup and Recovery"].
...
* Ao usar uma CR para restaurar em um novo namespace, você deve criar manualmente o namespace de destino antes de aplicar a CR. O NetApp Backup and Recovery cria namespaces automaticamente somente quando você usa a CLI.
...
NOTE: Se o BackupRestore CR não estiver disponível, você pode usar o comando da etapa 2 para visualizar o conteúdo do backup.
