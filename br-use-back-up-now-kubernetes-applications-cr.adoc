...
summary: 'Utilize recursos personalizados (CRs) para criar snapshots e backups sob demanda de um aplicativo Kubernetes, estabelecendo uma linha de base ou protegendo suas alterações de dados mais recentes.' 
---
= Faça backup de aplicativos Kubernetes agora usando recursos personalizados em NetApp Backup and Recovery
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Utilize recursos personalizados (CRs) para criar snapshots e backups sob demanda de um aplicativo Kubernetes, estabelecendo uma linha de base ou protegendo suas alterações de dados mais recentes.
...
** *spec.reclaimPolicy*: (_Opcional_) Define o que acontece com o AppArchive de um Snapshot quando o Snapshot CR é excluído. Mesmo quando definido como  `Retain`, o próprio Snapshot CR ainda é excluído; apenas o AppArchive é mantido. Opções válidas:
+
*** `Retain` (padrão)
*** `Delete`
+
...
** *spec.dataMover*: (_Opcional, obrigatório para clusters migrados do Trident Protect_) Uma string indicando qual ferramenta de backup usar para a operação de backup. Se este cluster foi migrado para Backup and Recovery do Trident Protect, o valor diferencia maiúsculas de minúsculas e deve ser  `CBS`.
...
| protect.trident.netapp.io/pvc-bind-timeout-sec | string | Tempo máximo (em segundos) para aguardar que quaisquer PersistentVolumeClaims (PVCs) recém-criadas atinjam a fase  `Bound` antes que a operação falhe. | "1200" (20 minutos) 
|===