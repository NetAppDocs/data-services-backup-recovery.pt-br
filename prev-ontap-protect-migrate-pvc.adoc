...
---
= Migrar o armazenamento do catálogo de pesquisa e restauração para o ONTAP NFS
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Para melhorar o desempenho de Busca e Restauração em uma implantação local do NetApp Console, migre o armazenamento do índice do catálogo da classe de armazenamento padrão do cluster para um compartilhamento NFS com suporte do ONTAP. O índice do catálogo oferece suporte a Busca e Restauração, e um NFS com suporte do ONTAP pode proporcionar melhorias de desempenho mensuráveis.
...
[discrete]
===== Monte o volume do ONTAP na implantação local do NetApp Console e copie os dados do pod
...
----
. Aplique os objetos YAML aplicando primeiro o PV e depois o PVC (o PV deve existir para que o seletor de rótulo do PVC possa ser vinculado). Em seguida, verifique se a declaração do PVC está vinculada:
+
[source, shell]
----
