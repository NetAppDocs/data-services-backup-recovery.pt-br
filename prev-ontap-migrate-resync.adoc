...
[role="lead"]
Use o SnapMirror to Cloud Resync no NetApp Backup and Recovery para manter os backups existentes na nuvem disponíveis após você migrar um volume com o SnapMirror Logical Replication (LRSE). Esse recurso permite que os backups continuem sem uma nova linha de base e está disponível a partir do ONTAP 9.16.1.

O SnapMirror to Cloud Resync oferece suporte às migrações de FlexVol e FlexGroup e ajuda você a manter a continuidade do backup em ambientes híbridos e de várias nuvens.
...
Imagine que você tenha um volume local chamado Vol1a. Este volume possui três snapshots: S1, S2 e S3. Esses snapshots são pontos de restauração. O Vol1a é copiado para a nuvem usando o SnapMirror to Cloud (SM-C), mas apenas S1 e S2 estão no armazenamento de objetos.
...
NOTE: Não utilize uma política SnapMirror do tipo "Vault" para clonar um volume de um cluster ONTAP antigo para um novo cluster ONTAP ou em uma configuração de arquitetura em cascata. As políticas Vault alteram o UUID da versão do snapshot durante a transferência. O UUID da versão do snapshot é usado para determinar se existe um snapshot comum entre a origem e o destino.
...
+
NetApp Backup and Recovery valida se cada volume selecionado para ressincronização compartilha um snapshot comum com o armazenamento de objetos. Especificamente, o snapshot mais recente armazenado no armazenamento de objetos também deve existir no snapshot local do novo volume clonado. Essa verificação garante que os volumes estejam prontos para a operação SnapMirror to Cloud Resync.
...
. Verifique a elegibilidade do volume. O sistema verifica se os volumes são elegíveis para ressincronização. Se um volume não for elegível, isso significa que os critérios comuns do Snapshot não foram atendidos.
+
