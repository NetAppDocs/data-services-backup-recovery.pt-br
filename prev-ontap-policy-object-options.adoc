...
[role="lead"]
Use políticas de backup para objetos no NetApp Backup and Recovery para definir agendamentos, retenção, DataLock, proteção contra ransomware e configurações de arquivamento para seus sistemas ONTAP locais e Cloud Volumes ONTAP.
...
Ambas essas funcionalidades protegem seus arquivos de backup, garantindo que você sempre tenha um arquivo de backup válido para recuperar dados em caso de uma tentativa de ataque de ransomware aos seus backups. Também é útil para atender a certos requisitos regulatórios em que os backups precisam ser bloqueados e retidos por um determinado período de tempo. Quando a opção *DataLock and Ransomware protection* está habilitada, o bucket na nuvem provisionado como parte da ativação do NetApp Backup and Recovery terá o bloqueio e o versionamento de objetos ativados.
...
+
** *Nenhum*: a proteção DataLock e a proteção contra ransomware estão desativadas.
** *Desbloqueado*: DataLock proteção e proteção contra ransomware estão ativadas. Usuários com permissões específicas podem sobrescrever ou excluir arquivos de backup protegidos durante o período de retenção.
** *Bloqueado*: DataLock proteção e proteção contra ransomware estão ativadas. Nenhum usuário pode sobrescrever ou excluir arquivos de backup protegidos durante o período de retenção. Isso atende integralmente à conformidade regulamentar.
...
| Cloud Volumes ONTAP no Google Cloud | Armazenamento em nuvem do Google 
...
* Para AWS:
+
** Seus clusters devem estar executando ONTAP 9.11.1 ou superior
...
* Para o Azure:
+
** Seus clusters devem estar executando ONTAP 9.12.1 ou superior
** O agente do Console pode ser implantado na nuvem ou em suas instalações
...
Ao usar o AWS, Azure ou Google Cloud Storage, você pode mover arquivos de backup mais antigos para uma classe de storage de arquivamento ou nível de acesso mais econômico após um determinado número de dias. Você também pode optar por enviar seus arquivos de backup para o storage de arquivamento imediatamente, sem gravá-los no storage de nuvem. Basta inserir *0* como "Archive After Days" para enviar seu arquivo de backup diretamente para o storage de arquivamento. Isso pode ser especialmente útil para usuários que raramente precisam acessar dados de backups na nuvem ou para usuários que estão substituindo uma solução de backup em fita.
...
+
** Para a AWS, você pode organizar os backups em camadas no armazenamento AWS _S3 Glacier_ ou _S3 Glacier Deep Archive_. link:prev-reference-aws-archive-storage-tiers.html["Saiba mais sobre o armazenamento de arquivo da AWS"^].
** Para o Azure, você pode organizar backups mais antigos em camadas no armazenamento _Azure Archive_. link:prev-reference-azure-archive-storage-tiers.html["Saiba mais sobre o armazenamento de arquivamento do Azure"^].
