...
Ambas as funcionalidades protegem seus arquivos de backup, garantindo que você sempre tenha um arquivo de backup válido para recuperar dados em caso de uma tentativa de ataque de ransomware aos seus backups. Elas também são úteis para atender a certos requisitos regulatórios em que os backups precisam ser bloqueados e retidos por um determinado período de tempo. Quando a opção *Datalock and Ransomware protection* está ativada, o bucket de nuvem provisionado como parte da ativação do NetApp Backup and Recovery terá o bloqueio de objetos e o versionamento de objetos habilitados.
...
. Ao criar uma política, expanda a seção *Datalock and Ransomware protection*.
