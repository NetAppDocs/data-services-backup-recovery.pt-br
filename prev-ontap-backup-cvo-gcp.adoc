...
---
= Faça backup do Cloud Volumes ONTAP para o Google Cloud Storage com o NetApp Backup and Recovery
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Use o NetApp Backup and Recovery para fazer backup dos dados de volume dos seus sistemas Cloud Volumes ONTAP no Google Cloud Storage.
...
Você também precisa de uma conta do Google Cloud para o espaço de storage onde seus backups serão armazenados.
...
Para usar a funcionalidade de Pesquisa e Restauração do NetApp Backup and Recovery, a função do agente do Console deve incluir permissões específicas para que ele possa acessar o serviço BigQuery do Google Cloud. Veja as permissões abaixo e siga as etapas caso precise modificar a política.
...
+
.. Selecione *Editar Projeto* e escolha um novo projeto se o que você deseja usar for diferente do projeto padrão (onde reside o agente do Console).
...
. *Serviços*: Deixe o NetApp Backup and Recovery ativado e selecione *Continuar*.
...
+
Se o destino do Google Cloud Storage para seus backups existir como um sistema na página *Sistemas* do Console, você pode arrastar o cluster ONTAP para o sistema do Google Cloud Storage.
...
** Para políticas de backup para objetos, configure o Datalock e a proteção contra ransomware. Para obter detalhes sobre o Datalock e a proteção contra ransomware, consulte link:prev-ontap-policy-object-options.html["Configurações de política de backup para objeto"].
** Selecione *Criar*.
...
+
Se você optar por usar suas próprias chaves gerenciadas pelo cliente, insira as informações do key ring e da chave.
