...
[role="lead"]
Use o NetApp Backup and Recovery para gerenciar backup de snapshot, replicação e storage de objetos para seus sistemas Cloud Volumes ONTAP e ONTAP locais. Você pode alterar agendamentos, ativar ou desativar backup de volume, pausar backups, excluir backups e cancelar o registro do serviço.
...
. Verifique o status das opções de proteção na coluna "Proteção existente". Os três ícones representam "Snapshots locais", "Volumes replicados" e "Backups no storage de objetos".
+
...
Consulte link:br-start-enable.html["Ative o NetApp Backup and Recovery para proteger os volumes"^] para obter instruções.
...
.Passos
. No menu *Volumes*, localize o volume para o qual deseja modificar as configurações de política e selecione o menu Ações. image:icon-action.png["Ícone de ações"] No final da linha, selecione *Editar estratégia de backup*.
...
+
Se você habilitou o DataLock e a proteção contra ransomware para backups na nuvem na política de backup ao ativar o NetApp Backup and Recovery para este cluster, você verá apenas outras políticas que foram configuradas com o DataLock. E se você não habilitou o DataLock e a proteção contra ransomware ao ativar o NetApp Backup and Recovery, você verá apenas outras políticas de backup na nuvem que não têm o DataLock configurado.
...
. Dependendo do tipo de ação de backup que você deseja configurar, selecione o botão no menu Ações em massa:
+
[cols="50,30"]
|===
...
Se você habilitou o _DataLock e a proteção contra ransomware_ ao ativar o NetApp Backup and Recovery para este cluster, o backup sob demanda também será configurado com o DataLock e o período de retenção será de 30 dias. As verificações de ransomware não são compatíveis com backups ad-hoc. link:prev-ontap-policy-object-options.html["Saiba mais sobre a proteção DataLock e Ransomware"^].

Quando você cria um backup ad-hoc, um snapshot é criado no volume de origem. Como esse snapshot não faz parte de um cronograma normal de snapshots, ele não será removido automaticamente. Você pode excluir manualmente esse snapshot do volume de origem após a conclusão do backup. Isso permitirá que os blocos relacionados a esse snapshot sejam liberados. O nome do snapshot começará com  `cbs-snapshot-adhoc-`. https://docs.netapp.com/us-en/ontap/san-admin/delete-all-existing-snapshot-copies-volume-task.html["Veja como excluir um snapshot usando a CLI do ONTAP"^].
...
Este recurso está disponível somente se o backup do volume foi criado a partir de um sistema com ONTAP 9.11.1 ou superior e se você habilitou _DataLock and Ransomware protection_ na política de backup para objeto.
...
. Selecione image:icon-actions-horizontal.gif["Ícone de ações"] o arquivo de backup do volume que você deseja verificar em busca de ransomware e selecione *Verificar Ransomware*.
+
A coluna de proteção contra ransomware mostra que a verificação está em andamento.
...
[NOTE]
====
* Se você habilitou _DataLock e proteção contra ransomware_ na política inicial ao ativar o NetApp Backup and Recovery para este cluster, todas as políticas que você editar devem ser configuradas com a mesma configuração de DataLock (Governance ou Compliance). E se você não habilitou _DataLock e proteção contra ransomware_ ao ativar o NetApp Backup and Recovery, não é possível habilitar DataLock agora.
...
[NOTE]
====
* Se você habilitou _DataLock and Ransomware protection_ na política inicial ao ativar o NetApp Backup and Recovery para este cluster, quaisquer políticas adicionais que você criar deverão ser configuradas com a mesma configuração DataLock (Governance ou Compliance). E se você não habilitou _DataLock and Ransomware protection_ ao ativar o NetApp Backup and Recovery, você não pode criar novas políticas que usem DataLock.
...
NOTE: Não é possível excluir arquivos de backup de volume individuais depois que o relacionamento de backup for excluído. Exclua os arquivos de backup de um volume antes de excluir o relacionamento de backup do volume.

.Passos
. No menu *Volumes*, selecione image:icon-actions-horizontal.gif["Mais ícone"] para o volume de origem e selecione *Gerenciar backups > Exibir backup*.
+
A lista de todos os arquivos de backup é exibida.

. Escolha um backup da lista e selecione *Ações > Excluir todos os backups*.
. Confirme a exclusão inserindo o nome do volume.
. Expanda a área *Configurações avançadas*.
. Em *Forçar exclusão de backups*, selecione se deseja forçar a exclusão de todos os backups.
+
Em casos raros, pode ser necessário revogar o acesso do NetApp Backup and Recovery aos backups; por exemplo, se o Backup and Recovery perder o acesso ao bucket de backup ou se os backups estiverem DataLock-protegidos, mas você não os quiser mais. Use esta opção para forçar a exclusão de backups no nível do volume e do sistema.

+

CAUTION: Use esta opção apenas para limpeza extrema. Depois de ativado, NetApp Backup and Recovery não acessará mais esses backups (mesmo que os objetos permaneçam no storage de nuvem). Você deve excluí-los manualmente do seu provedor de nuvem.
...
Exclua um backup individual quando ele não for mais necessário. Não é possível excluir volumes replicados (volumes de proteção de dados).


NOTE: Não é possível excluir arquivos de backup de volume individuais depois que o relacionamento de backup for excluído. No entanto, você pode <<Excluir todos os arquivos de backup de um volume,Excluir todos os arquivos de backup do volume>>.
...
NOTE: Não é possível excluir arquivos de backup de volume individuais depois que o relacionamento de backup for excluído. No entanto, você pode <<Excluir todos os arquivos de backup de um volume,Excluir todos os arquivos de backup do volume>>. Se precisar excluir um relacionamento de backup para um volume, certifique-se de excluir primeiro os backups do volume e, em seguida, excluir o relacionamento de backup.
...
Desativar o NetApp Backup and Recovery para um sistema pausa os backups e restaurações de todos os seus volumes sem excluir os backups existentes. Isso permite pausar toda a atividade de backup e restauração por um período de tempo sem cancelar o registro do serviço de backup nesse sistema.
