...
[role="lead"]
No NetApp Backup and Recovery, você pode fazer backup dos dados dos volumes dos seus sistemas ONTAP locais para um sistema de storage secundário e para o Amazon S3. Use esta página para preparar seu ambiente e ativar os backups.
...
+
Se você estiver usando um IPspace diferente de "Default", talvez seja necessário criar uma rota estática para acessar o storage de objetos.
...
NOTE: A criptografia gerenciada pelo cliente é compatível apenas com Amazon FSx for NetApp ONTAP. Não é compatível com Cloud Volumes ONTAP para AWS.

Consulte  https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-setting-up-kms.html["Configure a criptografia gerenciada pelo cliente na AWS"^].
...
.Resultado
O NetApp Backup and Recovery começa a fazer os backups iniciais dos seus volumes. A transferência de linha de base do volume replicado e do arquivo de backup inclui uma cópia completa dos dados do sistema de armazenamento primário. As transferências subsequentes contêm cópias diferenciais dos dados primários contidos nos instantâneos.
