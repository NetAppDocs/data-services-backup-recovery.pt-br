...
[role="lead"]
No NetApp Backup and Recovery, você pode fazer backup dos dados dos volumes dos seus sistemas ONTAP locais para um sistema de storage secundário e para o Google Cloud Storage. Use esta página para preparar seu ambiente e ativar backups.
...
* Observe que, se você estiver usando um IPspace diferente do Default, talvez seja necessário criar uma rota estática para acessar o storage de objetos.
...
+
Se o destino do Google Cloud Storage para seus backups existir na página *Sistemas* do Console, você pode arrastar o cluster ONTAP para o Google Cloud Storage.
...
** *Chave de criptografia*: Se você criou um novo bucket do Google Cloud Storage, insira as informações da chave de criptografia fornecidas pelo provedor. Escolha se você vai usar as chaves de criptografia padrão do Google Cloud ou se vai escolher suas próprias chaves gerenciadas pelo cliente em sua conta do Google Cloud para gerenciar a criptografia dos seus dados.
+

NOTE: Se você escolheu um bucket existente do Google Cloud Storage, as informações de criptografia já estão disponíveis, portanto, você não precisa inseri-las agora.
