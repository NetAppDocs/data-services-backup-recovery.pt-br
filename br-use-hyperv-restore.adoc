...
Na página Restaurar, você pode restaurar um recurso, mesmo que não se lembre do nome exato, da localização em que ele reside ou da data em que estava em bom estado pela última vez. Você pode pesquisar o snapshot usando filtros.
...
Restaure cargas de trabalho do Hyper-V usando o menu Restore. Você pode pesquisar o snapshot pelo nome ou usando filtros.
...
.. Na seção *Armazenamento de destino* (visível somente ao restaurar a partir do armazenamento de objetos):
+
