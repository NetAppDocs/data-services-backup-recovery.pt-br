...
[role="lead"]
Descubra ou adicione manualmente destinos de backup externos, como AWS S3, Azure Blob Storage, Google Cloud Storage ou StorageGRID, para que você possa armazenar backups fora do seu ambiente principal.
...
Forneça as credenciais para acessar a conta de armazenamento. NetApp Backup and Recovery usa essas credenciais para descobrir e verificar o acesso aos seus buckets de destino de backup.

.Antes de começar
Você precisa descobrir pelo menos uma carga de trabalho antes de poder adicionar um destino de backup externo.

.Passos
. No menu NetApp Backup and Recovery, selecione *Destinos de backup*.
. Selecione *Descobrir destino de backup*.
. Selecione um dos tipos de destino de backup disponíveis: *AWS*, *Azure*, *StorageGRID*, *ONTAP S3* ou *GCP*.
. Selecione *Avançar*.
. (Apenas para destinos de backup da AWS e do Azure) Na seção *Escolher local das credenciais*, selecione o local onde as credenciais estão armazenadas e, em seguida, escolha como associar as credenciais.
. Selecione *Avançar*.
...
*** *Chave de acesso*: Insira a chave de acesso da AWS.
...
*** *ID do aplicativo (cliente)*: Insira o ID do aplicativo do Azure Blob Storage.
...
*** *Nome de domínio totalmente qualificado (FQDN) do nó de gateway*: Insira um FQDN para o nó de gateway do StorageGRID.
...
*** *Nome de domínio totalmente qualificado (FQDN) do nó de gateway*: Insira um FQDN para o nó de gateway S3 do ONTAP.
...
** Para GCP:
+
*** *Nome da credencial*: Insira o nome da credencial do Google Cloud.
*** *Chave de acesso*: Insira a chave de acesso do Google Cloud.
...
Adicione manualmente um bucket a um destino de backup externo em vez de deixar que o NetApp Backup and Recovery descubra os buckets automaticamente.

.Passos
. No menu NetApp Backup and Recovery, selecione *Destinos de backup*.
. Selecione um alvo e, à direita, selecione o ícone image:icon-action.png["Ícone de ações"] *Ações* e selecione *Adicionar bucket*.
...
** Para o Azure:
+
*** *Assinatura*: selecione o nome da assinatura do Azure.
*** *Grupo de recursos*: selecione o nome do grupo de recursos do Azure.
*** *Detalhes da instância*:
+
**** *Nome da conta de armazenamento*: Insira o nome da conta do Azure Blob Storage.
**** *Região do Azure*: insira a região do Azure para a conta de armazenamento.
**** *Tipo de desempenho*: selecione o tipo de desempenho, padrão ou premium, para a conta do Azure Blob Storage, indicando o nível de desempenho necessário.
**** *Criptografia*: selecione o tipo de criptografia para a conta do Azure Blob Storage. As opções são chaves gerenciadas pela Microsoft ou chaves gerenciadas pelo cliente. Se você selecionar chaves gerenciadas pelo cliente, você deve fornecer o nome do cofre de chaves e o nome da chave.
...
** Para StorageGRID:
+
*** *Destino de backup*: selecione o nome do destino de backup do StorageGRID.
...
** Para ONTAP S3:
+
*** *Destino de backup*: selecione o nome do destino de backup do ONTAP S3.
*** *Nome do bucket*: Insira o nome do bucket S3 do ONTAP.
...
** Para GCP:
+
*** *Nome do bucket*: Insira o nome do bucket do Google Cloud.
*** *Projeto GCP*: selecione o nome do projeto do Google Cloud.
*** *Região do bucket*: Selecione uma região para o bucket do Google Cloud.
*** *Bloqueio de objetos*: Selecione esta opção para ativar períodos de retenção para objetos neste bucket. Enquanto um período de retenção estiver em vigor, os objetos não poderão ser modificados ou excluídos e permanecerão imutáveis até o término do período. Se você ativar esta opção para um bucket, ela não poderá ser desativada.
...
.Passos
. No menu NetApp Backup and Recovery, selecione *Destinos de backup*.
. Selecione um destino e, à direita, selecione o ícone image:icon-action.png["Ícone de ações"] *Ações* e selecione *Alterar credenciais*.
