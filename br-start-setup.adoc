...
Depois de configurar as opções no próprio destino de backup, você o configurará posteriormente como um destino de backup no NetApp Backup and Recovery.  Para obter detalhes sobre como configurar o destino de backup no NetApp Backup and Recovery, consultelink:br-start-discover-backup-targets.html["Descubra alvos de backup"] .
