...
---
= Faça backup do Cloud Volumes ONTAP para o Amazon S3 com o NetApp Backup and Recovery
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Use o NetApp Backup and Recovery para fazer backup dos dados de volume dos seus sistemas Cloud Volumes ONTAP para o Amazon S3.
...
NOTE: Se você usar um endpoint VPC privado para transferências de dados de backup ou restauração, ele terá precedência sobre qualquer conexão proxy configurada.
...
Você também precisa de uma conta da AWS para o espaço de storage onde seus backups serão armazenados.
...
.Passos
. Na página *Sistemas* do Console, selecione o cluster e selecione *Ativar* ao lado de NetApp Backup and Recovery no painel direito.
+
Se o Amazon S3 como destino para seus backups existir como um sistema na página *Sistemas*, você pode arrastar o cluster para o sistema Amazon S3 para iniciar o assistente de instalação.
...
+
Se o Amazon S3 destino para seus backups existir como um sistema na página *Sistemas* do Console, você pode arrastar o cluster ONTAP para o sistema Amazon S3.

** Selecione *Volumes* na barra de Backup and Recovery. Na guia Volumes, selecione o ícone image:icon-action.png["Ícone de ações"] *Ações* e selecione *Ativar Proteção 3-2-1* para um único volume (que ainda não tenha replicação ou backup para storage de objetos ativado).
...
+
Se você optar por usar suas próprias chaves gerenciadas pelo cliente, insira as informações da chave.
...
. Opcionalmente, marque a caixa para *Corrigir automaticamente rótulos incompatíveis em snapshots locais, replicação e backup*. Isso cria snapshots com um rótulo que corresponde aos rótulos nas políticas de snapshot, replicação e backup.
