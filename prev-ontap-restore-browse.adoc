...
summary: Com NetApp Backup and Recovery, restaure os dados do ONTAP usando Navegar e Restaurar. 
---
= Restaure os dados do ONTAP usando a opção Procurar e Restaurar em NetApp Backup and Recovery
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Use a opção Procurar e Restaurar quando você souber o nome do volume de origem, o sistema de origem, a VM de armazenamento e a data do arquivo de backup. Você pode restaurar dados do ONTAP a partir de um snapshot, de um volume replicado ou de um backup armazenado no storage de objetos.
...
NOTE: A prioridade de restauração Alta não é compatível ao restaurar dados do armazenamento de arquivamento do Azure para o StorageGRID.
...
Você pode restaurar os dados do ONTAP a partir de um arquivo de backup que reside em um sistema secundário (um volume replicado) ou em storage de objetos (um arquivo de backup) para os seguintes sistemas.
...
| Storages de objetos compatíveis com S3 | Sistema ONTAP local  a| 
* Sistema ONTAP local
* Cloud Volumes ONTAP
...
* Para o Amazon S3, o agente do Console pode ser implantado na AWS ou em ambientes locais
* Para o Azure Blob, o agente do Console pode ser implantado no Azure ou em um ambiente local
* Para o Google Cloud Storage, o agente do Console deve ser implantado na sua VPC do Google Cloud Platform
* Para o StorageGRID, o agente do Console deve ser implantado localmente, com ou sem acesso à internet
* Para o ONTAP S3, o agente do Console pode ser implementado localmente (com ou sem acesso à internet) ou em um ambiente de provedor de nuvem


Observe que as referências a "sistemas ONTAP locais" incluem os sistemas FAS, AFF e ONTAP Select.
...
Ao restaurar um backup em um sistema Cloud Volumes ONTAP usando o ONTAP 9.13.0 ou superior ou em um sistema ONTAP local executando o ONTAP 9.14.1, você terá a opção de realizar uma operação de _restauração rápida_. A restauração rápida é ideal para situações de recuperação de desastres em que você precisa fornecer acesso a um volume o mais rápido possível. Uma restauração rápida restaura os metadados do arquivo de backup para um volume, em vez de restaurar o arquivo de backup inteiro. A restauração rápida não é recomendada para aplicativos sensíveis ao desempenho ou à latência, e não é compatível com backups em armazenamento arquivado.


NOTE: A restauração rápida é compatível com volumes FlexGroup somente se o sistema de origem do qual o backup em nuvem foi criado estava executando o ONTAP 9.12.1 ou superior. Ela é compatível com volumes SnapLock somente se o sistema de origem estava executando o ONTAP 9.11.0 ou superior.
...
. Selecione a aba *Restaurar* para exibir o Painel de Restauração.
...
+
A coluna *Localização* mostra se o arquivo de backup (snapshot) é *Local* (um snapshot no sistema de origem), *Secundário* (um volume replicado em um sistema ONTAP secundário) ou *Storage de Objetos* (um arquivo de backup no storage de objetos). Escolha o arquivo que você quer restaurar.

. Selecione *Avançar*.
+
Observe que, se você selecionar um arquivo de backup no storage de objetos e a proteção contra ransomware estiver ativa para esse backup (caso você tenha habilitado o DataLock e a proteção contra ransomware na política de backup), será solicitado que você execute uma verificação adicional de ransomware no arquivo de backup antes de restaurar os dados. Recomendamos que você verifique o arquivo de backup em busca de ransomware. (Você incorrerá em custos adicionais de saída do seu provedor de nuvem para acessar o conteúdo do arquivo de backup.)
...
+
** Ao restaurar do Amazon S3, selecione o IPspace no cluster ONTAP onde o volume de destino ficará, insira a access key e a secret key do usuário que você criou para conceder ao cluster ONTAP acesso ao bucket do S3 e, opcionalmente, escolha um endpoint VPC privado para transferência de dados segura. Se você usar um endpoint VPC privado, ele terá precedência sobre qualquer conexão proxy configurada.
...
. Opcionalmente, habilite *Priorizar esta restauração em relação à fila de tarefas existente* para dar maior prioridade a esta operação de restauração. Quando habilitado, outras tarefas em execução podem ser pausadas ou atrasadas para permitir que esta restauração prossiga.
. Selecione *Restaurar*.
+
Você retorna ao Painel de Restauração para que possa revisar o progresso da operação de restauração.
...
* Com o ONTAP 9.15.1, você pode restaurar pastas FlexGroup usando a opção "Procurar e restaurar".
+
...
+
Se a proteção contra ransomware estiver ativa para o arquivo de backup (caso você tenha habilitado o DataLock e a proteção contra ransomware na política de backup), você será solicitado a executar uma verificação adicional de ransomware no arquivo de backup antes de restaurar os dados. Recomendamos que você verifique o arquivo de backup em busca de ransomware. (Você incorrerá em custos adicionais de saída do seu provedor de nuvem para acessar o conteúdo do arquivo de backup.)
...
*** Você pode passar o cursor sobre uma pasta e selecionar o final da linha para acessar as subpastas e, em seguida, selecionar uma pasta.


** Se você selecionou o mesmo sistema de destino e volume que a pasta ou o local do arquivo de origem, pode selecionar *Manter Caminho da Pasta de Origem* para restaurar a pasta ou os arquivos para a mesma pasta onde eles existiam na estrutura de origem. Todas as mesmas pastas e subpastas já devem existir; pastas não são criadas. Ao restaurar os arquivos para o local original, você pode escolher sobrescrever os arquivos de origem ou criar novos arquivos.
