...
[role="lead"]
Com o NetApp Backup and Recovery, restaure dados do ONTAP usando a opção Browse & Restore. Antes de restaurar, anote o nome do volume de origem, o sistema de origem, a storage VM e a data do arquivo de backup. Você pode restaurar dados do ONTAP a partir de um Snapshot, de um volume replicado ou de backups armazenados em storage de objetos.
...
| NetApp StorageGRID | Sistema ONTAP local  a| 
* Sistema ONTAP local
* Cloud Volumes ONTAP
...
. Selecione *Avançar*.
+
Observe que, se você selecionar um arquivo de backup no storage de objetos e a proteção contra ransomware estiver ativa para esse backup (caso tenha habilitado o Datalock e a proteção contra ransomware na política de backup), será solicitado que você execute uma verificação adicional de ransomware no arquivo de backup antes de restaurar os dados. Recomendamos que você verifique o arquivo de backup em busca de ransomware. (Você incorrerá em custos adicionais de saída do seu provedor de nuvem para acessar o conteúdo do arquivo de backup.)
...
+
E se a proteção contra ransomware estiver ativa para o arquivo de backup (se você habilitou Datalock e proteção contra ransomware na política de backup), você será solicitado a executar uma verificação adicional de ransomware no arquivo de backup antes de restaurar os dados. Recomendamos que você verifique o arquivo de backup em busca de ransomware. (Você incorrerá em custos adicionais de saída do seu provedor de nuvem para acessar o conteúdo do arquivo de backup.)
