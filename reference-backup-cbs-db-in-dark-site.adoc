...
summary: 'No modo privado (sem acesso à internet), NetApp Backup and Recovery armazena seus dados de configuração no mesmo bucket StorageGRID ou ONTAP S3 usado para seus backups. Se o sistema host do agente do Console falhar, você pode implantar um novo agente do Console e restaurar esses dados.' 
---
...
[role="lead"]
No modo privado (sem acesso à internet), NetApp Backup and Recovery armazena seus dados de configuração no mesmo bucket StorageGRID ou ONTAP S3 usado para seus backups. Se o sistema host do agente do Console falhar, você pode implantar um novo agente do Console e restaurar esses dados.

Você também pode seguir estas etapas para substituir o agente do Console e restaurar os dados de configuração armazenados nele ao usar o Backup e Recovery no _modo padrão_ localmente, quando nenhuma conta de nuvem estiver envolvida (por exemplo, ao usar NetApp StorageGRID como destino de backup).
...
. Faça login no NetApp Console usando as credenciais de usuário administrador que você acabou de criar.
...
. Copie o backup do MySQL do local de backup para o novo host do agente do Console. O exemplo a seguir usa o nome de arquivo  `CBS_DB_Backup_23_05_2023.sql`.
...
----
. Certifique-se de que os volumes exibidos sejam os mesmos que existiam em seu ambiente original.
...
. Copie o arquivo ZIP de backup do Catálogo Indexado do local de backup para a pasta  `/opt/application/netapp/cbs` no novo host do agente do Console. O exemplo a seguir usa o nome de arquivo  `Indexed_Catalog_DB_Backup_catalogdb1_23_05_2023.zip`.
...
----
. Execute o comando *ls* para garantir que a pasta "catalogdb1" foi criada e que contém as subpastas `files`, `versions`, e `snapshots`.
...
Adicione os detalhes do sistema StorageGRID que está associado aos seus sistemas ONTAP, conforme foram configurados na configuração original do agente do Console, usando o  https://docs.netapp.com/us-en/console-automation/index.html["APIs do NetApp Console"^].
...
----
+
Embora o endereço IP, o nome de usuário e a senha sejam valores personalizados, o nome da conta não é. O nome da conta é sempre  `account-DARKSITE1`. Além disso, o nome de usuário deve ser um nome formatado para e-mail.

+
Esta API retorna uma resposta como a seguinte. Você pode recuperar o token de autorização conforme mostrado abaixo.
...
----
+
Esta API retorna uma resposta como a seguinte. O valor em  `resourceIdentifier` indica o _ID do WorkingEnvironment_ e o valor em  `agentId` indica _x-agent-id_.
...
----
. Atualize o banco de dados do NetApp Backup and Recovery com os detalhes do sistema StorageGRID que está associado aos sistemas. Certifique-se de inserir o nome de domínio totalmente qualificado do StorageGRID, juntamente com a chave de acesso e a chave de armazenamento, conforme mostrado abaixo:
+
[source, shell]
----
...
. Selecione cada sistema ONTAP e selecione *Exibir backups* ao lado do serviço NetApp Backup and Recovery no painel direito.
+
Você deverá ver todos os backups criados para seus volumes.

. No painel de restauração, na seção Pesquisar e restaurar, selecione *Configurações de indexação*.
+
Certifique-se de que os sistemas que tinham a Catalogação Indexada ativada anteriormente permaneçam ativados.
