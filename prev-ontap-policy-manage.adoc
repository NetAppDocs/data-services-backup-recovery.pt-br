...
[role="lead"]
No NetApp Backup and Recovery, você pode usar as políticas de backup padrão ou criar políticas personalizadas para seus volumes ONTAP. As políticas controlam a frequência de backup, o horário do backup e a retenção.
...
Ao usar o assistente de ativação para habilitar o serviço de backup e recuperação para seus volumes, você pode selecionar entre as políticas padrão e quaisquer outras políticas já existentes no sistema (Cloud Volumes ONTAP ou sistema ONTAP local). Se você quiser usar uma política diferente das políticas existentes, pode criar a política antes ou enquanto usa o assistente de ativação.
...
NetApp Backup and Recovery oferece três tipos de backup de dados do ONTAP: snapshots, replicações e backups para storage de objetos. As políticas correspondentes ficam em locais diferentes, dependendo da arquitetura que você utiliza e do tipo de backup:
...
. Na página de gerenciamento de políticas, selecione *Criar política* > *Criar política de snapshot*.
...
.Passos
. Na página de gerenciamento de políticas, selecione *Criar política* > *Criar política de replicação*.
...
.Passos
. Na página de gerenciamento de políticas, selecione *Criar política* > *Criar política de backup*.
. Na seção Detalhes da política, especifique o nome da política.
. Especifique os rótulos do SnapMirror (máximo de 5) que indicam a retenção de cada rótulo.
