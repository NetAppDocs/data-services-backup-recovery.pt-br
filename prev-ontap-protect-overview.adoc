...
---
= Saiba mais sobre o NetApp Backup and Recovery para volumes ONTAP
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
NOTE: As referências a "on-premises ONTAP system" incluem os sistemas FAS, AFF e ONTAP Select.
...
* Se você planeja verificar um arquivo de backup para ransomware durante o processo de restauração de dados do volume (caso tenha habilitado Datalock e Ransomware protection para seus backups na nuvem), também incorrerá em custos adicionais de saída de dados por parte do seu provedor de nuvem.
...
Se o seu cluster estiver usando ONTAP 9.11.1 ou superior, você pode proteger seus backups no storage de objetos contra exclusão e ataques de ransomware. Cada política de backup fornece uma seção para _Datalock and Ransomware protection_ que pode ser aplicada aos seus arquivos de backup por um período específico - o _período de retenção_.
