...
---
= Saiba mais sobre a proteção de volumes do ONTAP com NetApp Backup e Recuperação
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Use o NetApp Backup and Recovery para proteger e restaurar os dados do seu volume ONTAP e para arquivar backups para o longo prazo. Você pode implementar uma estratégia 3-2-1 com três cópias dos seus dados de origem em dois sistemas de storage diferentes e uma cópia na nuvem.
...
Após a ativação, o Backup e Recovery cria backups incrementais permanentes em nível de bloco, armazenados em outro cluster ONTAP e em storage de objetos na nuvem. Além do seu volume de origem, você terá:
...
image:diagram-321-overview-unified.png["Um diagrama mostrando como os arquivos de backup existem no sistema de origem como snapshots, como volumes replicados no sistema de armazenamento secundário e como arquivos de backup no armazenamento de objetos."]

O NetApp Backup and Recovery usa a tecnologia de replicação de dados SnapMirror da NetApp para manter seus backups sincronizados, criando snapshots e transferindo-os para os locais de backup.
...
* Configure a criptografia nativa em trânsito do ONTAP usando uma chave pré-compartilhada (PSK) entre os dois sistemas.
...
* Quando comparada ao https://docs.netapp.com/us-en/data-services-replication/index.html["NetApp Replication"^], a replicação no NetApp Backup and Recovery inclui os seguintes recursos:
+
...
* Analise os arquivos de backup em busca de possíveis ataques de ransomware e remova ou substitua automaticamente os backups infectados.
...
NetApp Backup e recuperação permite que você faça backup de volumes ONTAP dos seguintes sistemas de origem para os seguintes sistemas secundários e storage de objetos em provedores de nuvem pública e privada. Os snapshots residem no sistema de origem.


NOTE: Atualmente, a Wasabi é a única provedora de storage de objetos compatível com S3 qualificada. Outros provedores compatíveis com S3 ainda não foram validados.
...
| Cloud Volumes ONTAP no Azure  a| 
...
| Cloud Volumes ONTAP no Google  a| 
...
| Storage de objetos compatível com S3 | Sistema ONTAP local | Sistema ONTAP local 
|===

NOTE: As referências a "sistema ONTAP local" incluem os sistemas FAS, AFF e ONTAP Select.
...
* Se você planeja verificar um arquivo de backup para ransomware durante o processo de restauração dos dados do volume (caso tenha habilitado o DataLock e a proteção contra ransomware para seus backups na nuvem), você também incorrerá em custos adicionais de saída do seu provedor de nuvem.
...
Uma licença de backup é necessária apenas para backup e restauração do armazenamento de objetos. A criação de snapshots e volumes replicados não requer licença.
...
* Os _snapshots_ residem no volume de origem no sistema de origem.
...
+
** Na AWS, o Console permite o https://docs.aws.amazon.com/AmazonS3/latest/dev/access-control-block-public-access.html["Recurso de bloqueio de acesso público do Amazon S3"^] no bucket S3.
...
NOTE: A política de snapshot aplicada ao volume deve ter um dos rótulos que você está usando em sua política de replicação e na política de backup para storage de objetos. Se rótulos correspondentes não forem encontrados, nenhum arquivo de backup será criado. Por exemplo, se você quiser criar volumes replicados e arquivos de backup "semanais", deverá usar uma política de snapshot que crie snapshots "semanais".
...
Se o seu cluster estiver usando o ONTAP 9.11.1 ou superior, você pode proteger seus backups no storage de objetos contra exclusão e ataques de ransomware. Cada política de backup fornece uma seção para _DataLock e proteção contra ransomware_ que pode ser aplicada aos seus arquivos de backup por um período específico - o _período de retenção_.
...
O suporte está disponível atualmente quando o seu destino de backup é o Amazon S3, o Azure Blob ou o StorageGRID. Outros destinos de provedores de storage serão adicionados em versões futuras.
...
+
Se o seu cluster local estiver usando o ONTAP 9.12.1 ou superior e o seu sistema StorageGRID estiver usando a versão 11.4 ou superior, você pode arquivar arquivos de backup mais antigos em storage de arquivamento na nuvem pública após um determinado número de dias. O suporte atual é para os níveis de armazenamento AWS S3 Glacier/S3 Glacier Deep Archive ou Azure Archive. link:prev-ontap-backup-onprem-storagegrid.html["Saiba mais sobre como arquivar arquivos de backup do StorageGRID"].



Veja link:prev-ontap-policy-object-options.html["como funciona o arquivamento de arquivos de backup antigos"].
