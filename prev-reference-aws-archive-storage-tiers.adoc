...
[role="lead"]
Use essas informações para escolher um nível de arquivamento do AWS S3 para o NetApp Backup and Recovery backups. Você também pode consultar os custos de restauração e os tempos de recuperação dos backups arquivados.
...
Quando os arquivos de backup são criados inicialmente, eles são armazenados no armazenamento S3 _Standard_. Essa camada é otimizada para armazenar dados acessados com pouca frequência, mas ainda permite acesso imediato. Após 30 dias, os backups são transferidos para a classe de armazenamento S3 _Standard-Infrequent Access_ para economizar custos.
