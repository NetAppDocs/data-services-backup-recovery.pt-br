...
SnapCenter não mapeia clusters de storage ou VMs de storage para os recursos em hosts, mas o NetApp Backup and Recovery mapeia. O cluster ONTAP local ou a VM de storage não será mapeado para um host nas versões de prévia do NetApp Backup and Recovery. Além disso, o NetApp Console não oferece suporte a VMs de storage.
