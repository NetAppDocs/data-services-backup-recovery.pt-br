...
---
= Limitações conhecidas do NetApp Backup and Recovery para cargas de trabalho do SQL Server
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Analise estas limitações antes de usar o NetApp Backup and Recovery para cargas de trabalho do SQL Server.
...
Para obter detalhes sobre como as políticas do SnapCenter migram para as políticas de NetApp Backup and Recovery , consultelink:reference-policy-differences-snapcenter.html["Políticas no SnapCenter comparadas com aquelas no NetApp Backup and Recovery"] .
...
Você não pode atribuir várias políticas que usem o mesmo nível de agendamento (por exemplo, por hora, diariamente ou semanalmente) a um recurso. O NetApp Backup and Recovery não importará esses recursos do SnapCenter.
...
Se uma política do SnapCenter se repetir em intervalos diários e mensais, o NetApp Backup and Recovery não importará a política.

Por exemplo, você não pode associar uma política diária (com intervalo de 7 dias ou menos, ou superior a 7 dias) a um recurso e também associar uma política mensal ao mesmo recurso.
...
SnapCenter não mapeia clusters de armazenamento ou VMs de armazenamento para recursos em hosts, mas o NetApp Backup and Recovery mapeia. O cluster ONTAP local ou a VM de armazenamento não será mapeado para um host nas versões de prévia do NetApp Backup and Recovery. Além disso, o NetApp Console não oferece suporte a VMs de armazenamento.
