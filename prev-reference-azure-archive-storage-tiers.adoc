...
[role="lead"]
Use estas informações para entender o nível de arquivamento do Azure compatível com o NetApp Backup and Recovery. Você também pode consultar os custos de restauração e os tempos de recuperação de backups arquivados.
...
Quando os arquivos de backup são criados inicialmente, eles são armazenados na camada de acesso _Cool_. Essa camada é otimizada para armazenar dados que são acessados com pouca frequência, mas que podem ser acessados imediatamente quando necessário.
...
NOTE: A prioridade alta não é suportada ao restaurar dados do Azure para os sistemas StorageGRID.
