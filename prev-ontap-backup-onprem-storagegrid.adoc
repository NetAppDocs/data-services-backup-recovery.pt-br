...
[role="lead"]
No NetApp Backup and Recovery, você pode fazer backup dos dados de volume dos seus sistemas ONTAP locais primários para um sistema de storage secundário e para o StorageGRID storage de objetos. Use esta página para preparar seu ambiente e ativar os backups.
...
+
A nova versão do NetApp Backup e Recuperação, que inclui a capacidade de agendar e criar snapshots e volumes replicados, além de criar backups para storage de objetos, requer que você esteja usando a versão 3.9.31 ou superior do agente do Console. NetApp recomenda que você use esta versão mais recente para gerenciar todos os seus backups.
...
Para obter detalhes sobre DataLock e os requisitos de proteção contra ransomware para o StorageGRID, consulte link:prev-ontap-policy-object-options.html["Opções de política de backup para objeto"].

Versões do StorageGRID suportadas:: O StorageGRID 10.3 e versões posteriores são suportados.
+
--
Para utilizar DataLock & proteção contra ransomware em seus backups, seus sistemas StorageGRID devem estar executando a versão 11.6.0.3 ou superior.
...
Para utilizar o armazenamento de arquivamento, é necessário acesso ao endereço IP do nó de administração.
...
+
Se o seu cluster estiver usando o ONTAP 9.11.1 ou superior, você pode optar por proteger seus backups contra exclusão e ataques de ransomware configurando o _DataLock_ e a _proteção contra ransomware_. O _DataLock_ protege seus arquivos de backup contra modificações ou exclusões, e a _proteção contra ransomware_ verifica seus arquivos de backup em busca de evidências de um ataque de ransomware nos seus arquivos de backup.
