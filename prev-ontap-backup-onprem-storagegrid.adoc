...
Para obter detalhes sobre os requisitos de proteção contra Datalock e Ransomware para StorageGRID, consulte link:prev-ontap-policy-object-options.html["Opções de política de backup para objeto"].
...
+
Se o seu cluster estiver usando ONTAP 9.11.1 ou superior, você pode optar por proteger seus backups contra exclusão e ataques de ransomware configurando _DataLock e proteção contra ransomware_. _DataLock_ protege seus arquivos de backup contra modificações ou exclusões, e _proteção contra ransomware_ verifica seus arquivos de backup em busca de evidências de um ataque de ransomware em seus arquivos de backup.
