...
---
= Modificar recursos do Kubernetes durante a restauração em NetApp Backup and Recovery
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Use transformações de recursos para modificar recursos do Kubernetes durante operações de restauração. Para obter informações sobre como modificar recursos na interface web, consulte link:br-use-restore-kubernetes-applications.html#restore-kubernetes-applications["Modificar recursos durante a restauração"^].
...
O exemplo a seguir mostra a estrutura básica de um objeto de restauração que contém transformações de recursos:
...
Use a  `replace`operação para substituir um valor existente em um recurso no caminho especificado. O caminho do ponteiro JSON já deve existir. O exemplo a seguir altera o nome do host de um objeto Route:
