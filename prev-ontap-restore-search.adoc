...
[role="lead"]
Você pode usar a função Pesquisar e Restaurar para recuperar volumes, pastas ou arquivos de backup do ONTAP. A função Pesquisar e Restaurar permite pesquisar em todos os backups (incluindo snapshots locais, volumes replicados e storage de objetos) sem a necessidade de nomes exatos de sistema, volume ou arquivo.
