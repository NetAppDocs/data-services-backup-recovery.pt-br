...
---
= Restaure os dados do ONTAP usando Pesquisar e Restaurar em NetApp Backup and Recovery
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Use a função Pesquisar e Restaurar para recuperar volumes, pastas ou arquivos de backups do ONTAP quando você não souber o nome exato do sistema, volume ou arquivo. Você pode pesquisar em snapshots locais, volumes replicados e backups de storage de objetos.
...
* Ao restaurar um volume com SnapLock habilitado, o volume restaurado mantém a proteção SnapLock somente se o armazenamento de destino tiver WORM habilitado. Se você restaurar para um destino que não seja WORM, o status SnapLock será removido do volume restaurado.


====
Antes de começar, você deve ter uma ideia do nome ou da localização do volume ou arquivo que você deseja restaurar.
...
| StorageGRID | Sistema ONTAP local Cloud Volumes ONTAP | Sistema ONTAP local 
...
** Você deve configurar uma LIF de dados NFS IPv4 na VM de armazenamento e uma exportação NFS para o volume. Busca e Restauração é compatível com volumes NFS e SMB/CIFS.
** O SnapDiff RPC Server deve ser ativado na storage VM. O Console faz isso automaticamente quando você habilita a indexação no sistema. (SnapDiff é a tecnologia que identifica rapidamente as diferenças de arquivos e diretórios entre snapshots.)
...
* Requisitos do Azure:
+
** Você deve registrar o Provedor de Recursos do Azure Synapse Analytics (chamado "Microsoft.Synapse") em sua assinatura. https://learn.microsoft.com/en-us/azure/azure-resource-manager/management/resource-providers-and-types#register-resource-provider["Veja como registrar este provedor de recursos para sua assinatura"^] Você deve ser o *Proprietário* ou *Colaborador* da assinatura para registrar o provedor de recursos.
...
NOTE: O recurso de Backup and Recovery não indexa nenhum snapshot cujo carimbo de data/hora de criação corresponda exatamente a outro snapshot do mesmo volume.
...
.Etapas para habilitar a indexação em um sistema:
Se a indexação já estiver habilitada para seu sistema, vá para a próxima seção para restaurar seus dados.

Primeiro, monte um volume separado para armazenar os arquivos de catálogo. Isso ajuda a evitar perda de dados caso os arquivos de catálogo de snapshots fiquem muito grandes. Isso não é necessário em todos os clusters; você pode montar qualquer volume de qualquer um dos clusters em seu ambiente. Se você não fizer isso, a indexação pode não funcionar corretamente.
...
[[mount-volume-steps]]
.Etapa para montar o volume para reindexação do catálogo
...
.. Na área _Filtrar por_, selecione os critérios de filtro. Por exemplo, você pode selecionar o sistema onde os dados residem e o tipo de arquivo, como um arquivo .JPEG. Você também pode selecionar o tipo de local de backup se quiser pesquisar apenas em snapshots disponíveis ou arquivos de backup no storage de objetos.
...
+
Observe que os resultados identificam snapshots de volumes locais e volumes replicados remotos que contêm o arquivo da sua pesquisa. Você pode escolher restaurar a partir do arquivo de backup na nuvem, do snapshot ou do volume replicado.
...
+
*** Ao restaurar a partir do Amazon S3, selecione o IPspace no cluster ONTAP onde o volume de destino residirá, insira a chave de acesso e a chave secreta do usuário que você criou para conceder ao cluster ONTAP acesso ao bucket do S3 e, opcionalmente, escolha um endpoint VPC privado para transferência de dados segura. Se você usar um endpoint VPC privado, ele terá precedência sobre qualquer conexão proxy configurada. link:prev-ontap-backup-onprem-aws.html["Veja detalhes sobre esses requisitos"].
