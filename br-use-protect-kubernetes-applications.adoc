...
---
= Adicione e proteja aplicativos Kubernetes em NetApp Backup and Recovery
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Use a interface web do NetApp Backup and Recovery ou arquivos de recursos personalizados para adicionar aplicativos Kubernetes para proteção. Você pode adicionar aplicativos baseados em namespace que usam recursos padrão do Kubernetes ou aplicativos baseados em máquina virtual que usam uma ou mais máquinas virtuais.
...
Para proteger uma aplicação Kubernetes, primeiro crie a aplicação no NetApp Backup and Recovery. Ao criar a aplicação, o NetApp Backup and Recovery passa a reconhecer a aplicação em execução no cluster Kubernetes.
...
[role="tabbed-block"]
====
.Adicionar um aplicativo baseado em namespace (interface web)
--
.Passos
. No NetApp Backup and Recovery, selecione *Inventário*.
. No canto superior direito da página, certifique-se de que *Kubernetes* esteja selecionado na lista de workloads.
. Selecione *Exibir* no item de linha da carga de trabalho para visualizar os recursos do Kubernetes.
...
NOTE: O Backup and Recovery não armazena os parâmetros ou resultados da pesquisa; os parâmetros são usados para pesquisar no cluster Kubernetes selecionado recursos que podem ser incluídos no aplicativo.
...
. Opcionalmente, na área *Política*, escolha uma política de proteção existente para proteger o aplicativo ou crie uma nova política. Se você não selecionar uma política, o aplicativo será criado sem uma política de proteção. Você pode link:br-use-policies-kubernetes-create.html#create-a-policy["adicionar uma política de proteção"] depois.
...
--
.Adicionar um aplicativo baseado em máquina virtual (web UI)
--
.Passos
. No NetApp Backup and Recovery, selecione *Inventário*.
. No canto superior direito da página, certifique-se de que *Kubernetes* esteja selecionado na lista de workloads.
...
NOTE: Se você selecionar VMs da lista, a definição do aplicativo será estática — novas VMs não serão adicionadas ao aplicativo posteriormente (você precisa editar o aplicativo para adicioná-las e protegê-las). Se você usar seletores de rótulo, não pode escolher VMs individuais nem editar a lista gerada, mas qualquer VM que corresponda ao seletor posteriormente é incluída automaticamente e protegida.

+
As máquinas virtuais selecionadas aparecem na lista à direita.
...
.Resultado
O aplicativo é criado e aparece na lista de aplicativos na guia *Aplicativos* do inventário do Kubernetes. NetApp Backup and Recovery permite a proteção do aplicativo com base nas suas configurações, e você pode monitorar o progresso na área *Monitoramento*.

--
.Adicionar um aplicativo baseado em namespace (CR)
--
.Passos
