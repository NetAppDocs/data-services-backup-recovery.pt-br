...
---
= Planeje a proteção de volume ONTAP no NetApp Backup e Recuperação
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Use esta página para planejar como você protegerá seus volumes do ONTAP com o NetApp Backup and Recovery. Analise os métodos de proteção, arquiteturas, políticas, opções de storage de objetos e modos de implantação do agente do Console antes de habilitar o backup e recuperação.
...
Antes de selecionar os recursos que você usará, aqui está uma breve explicação do que cada recurso faz e o tipo de proteção que oferece.
...
|===
Os snapshots são a base de todos os métodos de backup e são necessários para usar o serviço de backup e recuperação. Um snapshot é uma imagem pontual somente leitura de um volume. A imagem consome espaço mínimo de storage e acarreta uma sobrecarga de desempenho insignificante, pois registra apenas as alterações feitas nos arquivos desde que o último snapshot foi criado. O snapshot criado no seu volume é usado para manter o volume replicado e o arquivo de backup sincronizados com as alterações feitas no volume de origem, conforme mostrado na figura.

image:diagram-321-overview.png["Um diagrama mostrando como os arquivos de backup existem no sistema de origem como snapshots, como volumes replicados no sistema de armazenamento secundário e como arquivos de backup no armazenamento de objetos."]
...
Além desses critérios, o storage de nuvem oferece opções de segurança adicionais se você usar o recurso *DataLock and Ransomware protection*, e economia adicional ao selecionar classes de storage de arquivamento para arquivos de backup mais antigos. link:prev-ontap-policy-object-options.html["Saiba mais sobre a proteção do DataLock e do Ransomware e as configurações de armazenamento de arquivamento"].
...
image:diagram-321-fanout-detailed.png["Um diagrama que mostra como um instantâneo de um volume é usado para criar e atualizar um volume replicado e um arquivo de backup."]
...
image:diagram-321-cascade-detailed.png["Um diagrama que mostra como um instantâneo de um volume é usado para criar e atualizar um volume replicado e um arquivo de backup."]
...
== Você usará as políticas padrão para snapshots, replicação e backups

Você pode usar as políticas padrão fornecidas pela NetApp para criar seus backups ou pode criar políticas personalizadas. Ao usar o assistente de ativação para habilitar o serviço de backup e recuperação para seus volumes, você pode selecionar entre as políticas padrão e quaisquer outras políticas já existentes no sistema (Cloud Volumes ONTAP ou sistema ONTAP local). Se você quiser usar uma política diferente das políticas existentes, pode criá-la antes de iniciar ou durante o uso do assistente de ativação.
...
Você pode criar políticas de snapshot, replicação e backup para storage de objetos na interface do usuário do NetApp Backup and Recovery. Consulte link:prev-ontap-backup-manage.html["adicionando uma nova política de backup"] para obter detalhes.

Além de usar NetApp Backup and Recovery para criar políticas personalizadas, você pode usar System Manager ou a ONTAP Command Line Interface (CLI):
...
As políticas de backup residem em locais diferentes dependendo se você usa uma arquitetura fan-out ou em cascata. As políticas de replicação e as políticas de backup são projetadas de forma diferente porque a replicação emparelha dois sistemas de armazenamento ONTAP, enquanto o backup para objeto usa um provedor de armazenamento como destino.
...
|===
Portanto, se você planeja criar políticas personalizadas ao usar a arquitetura em cascata, precisará criar as políticas de replicação e de backup para objeto no sistema secundário onde os volumes replicados serão criados. Se você planeja criar políticas personalizadas ao usar a arquitetura fan-out, precisará criar as políticas de replicação no sistema secundário onde os volumes replicados serão criados e as políticas de backup para objeto no sistema primário.
...
Quando você cria arquivos de backup no storage de objetos para um sistema, por padrão, o serviço de backup e recuperação cria o contêiner (bucket ou conta de armazenamento) para os arquivos de backup na conta de storage de objetos que você configurou. O bucket da AWS ou do GCP é nomeado "netapp-backup-<uuid>" por padrão. A conta de armazenamento de blobs do Azure é nomeada "netappbackup<uuid>".
...
NOTE: Ao criar um contêiner para usar como destino do backup no Azure, certifique-se de que o contêiner que você criar tenha o mesmo nome que a conta de armazenamento do Azure.
...
Se você já utiliza o Console para gerenciar seu storage, um agente do Console já está instalado. Se você planeja usar o mesmo agente do Console com o NetApp Backup and Recovery, está tudo pronto. Se você precisar usar um agente do Console diferente, instale-o antes de iniciar a implementação do backup e recuperação.
...
* Você pode fazer backup de dados de sistemas ONTAP locais para sistemas StorageGRID locais. link:prev-ontap-backup-onprem-storagegrid.html["Faça backup de dados ONTAP locais no StorageGRID"].
* Você pode fazer backup de dados de sistemas ONTAP locais para sistemas ONTAP locais ou sistemas Cloud Volumes ONTAP configurados para armazenamento de objetos S3. link:prev-ontap-backup-onprem-ontaps3.html["Faça backup de dados ONTAP locais no ONTAP S3"].
