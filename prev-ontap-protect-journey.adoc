...
---
= Opções de planejamento da jornada de proteção no NetApp Backup and Recovery
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
Além desses critérios, storage de nuvem oferece opções de segurança adicionais se você usar o recurso *Datalock and Ransomware protection*, e economia adicional ao selecionar classes de armazenamento de arquivamento para arquivos de backup mais antigos. link:prev-ontap-policy-object-options.html["Saiba mais sobre a proteção do DataLock e do Ransomware e as configurações de armazenamento de arquivamento"].
