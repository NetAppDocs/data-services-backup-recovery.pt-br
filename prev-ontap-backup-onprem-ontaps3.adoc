...
[role="lead"]
No NetApp Backup and Recovery, você pode fazer backup dos dados de volume dos seus sistemas ONTAP locais primários para o ONTAP S3 e, opcionalmente, para um sistema ONTAP secundário para replicação. Use esta página para preparar seu ambiente e ativar backups.
...
+
** *Destino da replicação*: Selecione o sistema de destino e a máquina virtual de storage. Opcionalmente, selecione o agregado de destino (ou agregados para volumes FlexGroup) e um prefixo ou sufixo que será adicionado ao nome do volume replicado.
