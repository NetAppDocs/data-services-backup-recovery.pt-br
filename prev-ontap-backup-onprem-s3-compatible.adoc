...
[role="lead"]
Use esta página para verificar se sua configuração é compatível e para configurar backups dos seus sistemas ONTAP locais primários para storage de objetos compatível com S3 no NetApp Backup and Recovery.
...
+
** Na página *Sistemas* do Console, selecione o sistema e selecione *Ativar > Volumes de backup* ao lado de NetApp Backup and Recovery no painel direito.
...
TIP: Se você deseja criar uma política personalizada antes de ativar o snapshot, pode usar o System Manager ou o comando da CLI do ONTAP `snapmirror policy create`. Consulte .
...
*** Digite o nome da política.
*** Selecione até cinco programações, normalmente com frequências diferentes.
*** Para políticas de backup para objetos, defina as configurações de DataLock e proteção contra ransomware. Para obter detalhes sobre DataLock e proteção contra ransomware, consulte link:prev-ontap-policy-object-options.html["Configurações de política de backup para objeto"].
*** Selecione *Criar*.


** *Exportar snapshots existentes para o storage de objetos como arquivos de backup*: Se houver snapshots locais para volumes neste sistema que correspondam ao rótulo de agendamento de backup que você acabou de selecionar (por exemplo, diário, semanal etc.), esta mensagem adicional será exibida. Marque esta caixa para que todos os snapshots históricos sejam copiados para o storage de objetos como arquivos de backup, garantindo a proteção mais completa para seus volumes.
...
. Opcionalmente, marque a caixa para *Sincronizar automaticamente os rótulos da política de snapshot com os rótulos das políticas de replicação e de backup*. Isso cria snapshots com um rótulo que corresponde aos rótulos nas políticas de replicação e de backup. Se as políticas não corresponderem, os backups não serão criados.
...
Você pode exibir e, opcionalmente, copiar os comandos da API usados no assistente de ativação do NetApp Backup and Recovery. Você pode querer fazer isso para automatizar a ativação de backup em sistemas futuros.

.Passos
. No assistente Ativar backup e recuperação, selecione *Visualizar solicitação de API*.
