...
* Você pode restaurar uma *pasta* de snapshots replicados ou de storage de objetos para um volume no sistema original, para um volume em um sistema diferente que esteja usando a mesma conta na nuvem ou para um volume em um sistema ONTAP local. A restauração de pastas a partir de snapshots locais não é suportada.
