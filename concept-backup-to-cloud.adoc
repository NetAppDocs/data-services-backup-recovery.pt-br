...
[role="lead"]
Use esta visão geral para saber como o NetApp Backup and Recovery protege suas workloads do ONTAP, quais workloads e destinos de backup ele suporta e como funcionam o licenciamento e os custos.
...
O suporte para backup e recuperação já está integrado em todos os sistemas ONTAP, então não há necessidade de hardware adicional ou gateways de mídia. Isso torna as operações de backup simples e econômicas. O NetApp Console simplifica a implementação de qualquer estratégia de backup, incluindo todo o espectro de variantes de backup 3-2-1, sem a necessidade de múltiplos gerenciadores de recursos ou pessoal especializado.
...
** Faça backup das cargas de trabalho do VMware em provedores de armazenamento compatíveis.
...
** Consulte link:br-use-oracle-protect-overview.html["Proteger cargas de trabalho do Oracle Database"].
...
** No Google Cloud, um novo bucket é implantado e os  https://cloud.google.com/bigquery["Serviços do Google Cloud BigQuery"^] são provisionados no nível da conta/projeto.
...
* Se você planeja verificar se um arquivo de backup está infectado por ransomware durante o processo de restauração de dados do volume (caso tenha habilitado a proteção Datalock e Ransomware para seus backups na nuvem), você também incorrerá em custos adicionais de saída de dados do seu provedor de nuvem.
...
* A terceira opção é adquirir licenças diretamente da NetApp. Consulte a seção <<Licenciamento>> para obter detalhes.
...
* Google Cloud Storage (somente volumes ONTAP e cargas de trabalho do Kubernetes)
* StorageGRID
* ONTAP S3
* Armazenamento compatível com S3 (somente para cargas de trabalho de volumes ONTAP)
...
* No Azure, o NetApp Console usa um grupo de recursos novo ou existente com uma conta de armazenamento para o contêiner de Blob. O Console https://docs.microsoft.com/en-us/azure/storage/blobs/anonymous-read-access-prevent["bloqueia o acesso público aos seus dados de blob"] por padrão.
...
== Termos para NetApp Backup and Recovery
