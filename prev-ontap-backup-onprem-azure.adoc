...
[role="lead"]
No NetApp Backup and Recovery, você pode fazer backup dos dados de volume dos seus sistemas ONTAP locais para um sistema de storage secundário e para o Azure Blob. Use esta página para preparar seu ambiente e ativar backups.
...
+
O NetApp Backup and Recovery é compatível com as regiões do Azure Government quando o agente do Console é implantado na nuvem, não quando está instalado no seu ambiente local. Além disso, você deve implantar o agente do Console a partir do Azure Marketplace. Você não pode implantar o agente do Console em uma região do Government a partir do site SaaS do NetApp Console.
...
+
** Para o licenciamento PAYGO do NetApp Backup and Recovery, você precisará de uma assinatura da  https://marketplace.microsoft.com/en-us/product/netapp.cloud-manager?tab=Overview["Oferta do NetApp Console do Azure Marketplace"^]. A cobrança do NetApp Backup and Recovery é feita por meio dessa assinatura.
...
+
Observe que o NetApp Backup and Recovery oferece suporte às _políticas de acesso do Azure_ como modelo de permissão. O modelo de permissão _controle de acesso baseado em funções do Azure_ (Azure RBAC) não é compatível no momento.
...
+
** Na página *Sistemas* do Console, selecione o sistema e selecione *Ativar > Volumes de Backup* ao lado do serviço backup e recuperação no painel direito.
+
...
+
A página de Introdução do assistente mostra as opções de proteção, incluindo snapshots locais, replicação e backups. Se você selecionou a segunda opção nesta etapa, a página Definir estratégia de backup é exibida com um volume selecionado.
...
NOTE: Se você selecionar *Traga seu próprio bucket*, certifique-se de que o contêiner que você criar como destino de backup tenha o mesmo nome da conta de armazenamento do Azure.
...
. Opcionalmente, marque a caixa para *Sincronizar automaticamente os rótulos da política de snapshot com os rótulos das políticas de replicação e de backup*. Isso cria snapshots com um rótulo que corresponde aos rótulos nas políticas de replicação e de backup.
