...
** *Cascata 3-2-1*: (Não disponível para cargas de trabalho do Kubernetes) storage primário (disco) para storage secundário (disco) e storage primário (disco) para storage de nuvem (armazenamento de objetos). Isso pode ser um armazenamento de objetos de hyperscaler de nuvem ou um armazenamento de objetos privado, como StorageGRID. Isso cria uma cadeia de replicação de dados em vários sistemas para garantir redundância e confiabilidade. Esta opção não está disponível para Amazon FSx for NetApp ONTAP.
+
...
** *Fanout de disco para disco*: (Não disponível para cargas de trabalho do Kubernetes) storage primário (disco) para storage secundário (disco) e storage primário (disco) para storage secundário (disco). Você pode configurar várias configurações secundárias para a opção de fanout de disco para disco.
+
...
** *Provedor*: (Somente para cargas de trabalho do Kubernetes) Selecione o provedor de storage que hospeda os recursos do aplicativo Kubernetes e insira as credenciais para autenticar com o provedor.
...
+
** *Configurações do catálogo RMAN*: (aplica-se somente a cargas de trabalho do Oracle Database) Habilite a opção *Catalogar backup com o Oracle Recovery Manager (RMAN)* para catalogar automaticamente os metadados das operações de backup e recuperação do banco de dados Oracle. Os metadados são armazenados de acordo com as configurações de *Configurar catálogo RMAN* escolhidas para o banco de dados (no arquivo de controle por padrão). Consulte link:br-start-configure-oracle-database.html["Configurar um banco de dados Oracle"] para alterar essas configurações para um banco de dados.
...
** *Repetições de backup*: (Não aplicável a cargas de trabalho VMware) Para repetir a tarefa em caso de falha ou interrupção, selecione *Habilitar repetições de tarefas durante falhas*. Insira o número máximo de repetições de tarefas de snapshot e backup e o intervalo de tempo entre as repetições. O número de repetições deve ser inferior a 10.
+
