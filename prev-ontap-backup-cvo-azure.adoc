...
---
= Faça backup do Cloud Volumes ONTAP para o armazenamento de blobs do Azure com o NetApp Backup and Recovery
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Use o NetApp Backup and Recovery para fazer backup dos dados de volume dos seus sistemas Cloud Volumes ONTAP no Azure Blob storage.
...
Você também precisa de uma assinatura do Azure para o espaço de storage onde seus backups serão armazenados.
...
Para usar a funcionalidade de Pesquisa e Restauração do NetApp Backup and Recovery, a função do agente do Console deve incluir permissões específicas para que ele possa acessar o workspace do Azure Synapse e a conta do Azure Data Lake Storage. Veja as permissões abaixo e siga as etapas se você precisar modificar a política.
...
Você pode habilitar o NetApp Backup and Recovery em um novo ou existente Cloud Volumes ONTAP. Os passos variam ligeiramente para cada opção.
...
.Passos
. Na página *Sistemas* do Console, selecione o sistema e selecione *Ativar* ao lado de NetApp Backup and Recovery no painel direito.
+
...
Ative os backups a qualquer momento diretamente do seu Cloud Volumes ONTAP.
...
+
A página de Introdução do assistente exibe as opções de proteção, incluindo snapshots locais, replicação e backups. Se você usou a segunda opção nesta etapa, a página Definir Estratégia de Backup é exibida com um volume selecionado.
...
Selecione os volumes que você deseja proteger. Um volume protegido é aquele que possui uma ou mais das seguintes opções: política de snapshot, política de replicação, política de backup para storage de objetos.
...
NOTE: Se você escolheu uma conta de armazenamento do Azure existente, as informações de criptografia já estão disponíveis, portanto, você não precisa inseri-las agora.
...
O painel de controle de backup de volume é exibido para que você possa monitorar o estado dos backups.
