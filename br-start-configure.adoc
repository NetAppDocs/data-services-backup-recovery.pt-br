...
* <<Adicionar credenciais para recursos do host>> para qualquer host Windows, Microsoft SQL Server, Oracle Database ou Linux com o qual o NetApp Backup and Recovery precise se autenticar. Isso inclui as credenciais do sistema operacional convidado usadas ao restaurar arquivos ou pastas.
...
Crie modelos de transformação de recursos para armazenar as transformações comuns que você realiza nos recursos ao restaurá-los.

.Passos
. No menu NetApp Backup and Recovery , selecione *Configurações*.
. Selecione a seta para baixo para expandir a seção *Resource modifer template*.
...
.. Opcionalmente, insira um grupo de recursos, versão, nome.
