...
summary: Faça backup dos dados do aplicativo Microsoft SQL Server de sistemas ONTAP locais para Amazon Web Services, Microsoft Azure e StorageGRID para garantir que seus dados estejam protegidos. 
---
...
[role="lead"]
Faça backup das cargas de trabalho do Microsoft SQL Server de sistemas ONTAP locais para Amazon Web Services, Microsoft Azure ou StorageGRID.

* Crie políticas para fazer backup de cargas de trabalho de acordo com uma programação e gerencie as operações de backup e restauração. Consulte link:br-use-policies-create.html["Criar políticas"] para obter instruções.
* Configure o diretório de log para hosts descobertos antes de iniciar um backup.
* Crie um backup sob demanda para fazer backup das cargas de trabalho imediatamente.
...
*Função necessária do NetApp Console*: superadministrador do NetApp Backup and Recovery, administrador de backup do NetApp Backup and Recovery ou administrador de restauração do NetApp Backup and Recovery. Se você estiver protegendo dados armazenados em um cluster do Cloud Volumes ONTAP, a função de administrador do Cloud Volumes ONTAP também é necessária. Saiba mais link:reference-roles.html["Funções e privilégios de backup e recuperação"]. https://docs.netapp.com/us-en/console-setup-admin/reference-iam-predefined-roles.html["Saiba mais sobre as funções de acesso do NetApp Console para todos os serviços"^].
...
. Selecione os hosts ou nós nos quais você deseja armazenar os logs.
+
...
*Função necessária do NetApp Console* Superadministrador de Backup and Recovery ou função de administrador de backup do Backup and Recovery. Se você estiver protegendo dados armazenados em um cluster do Cloud Volumes ONTAP, a função de administrador do Cloud Volumes ONTAP também é necessária. Saiba mais link:reference-roles.html["Funções e privilégios de backup e recuperação"]. https://docs.netapp.com/us-en/console-setup-admin/reference-iam-predefined-roles.html["Saiba mais sobre as funções de acesso do NetApp Console para todos os serviços"^].
...
+
Se você deseja criar uma política, selecione *Criar nova política* e siga as instruções. Consulte link:br-use-policies-create.html["Criar políticas"] para mais informações.
...
*Função necessária do NetApp Console* Função de superadministrador de NetApp Backup and Recovery ou função de administrador de backup de NetApp Backup and Recovery. Se você estiver protegendo dados armazenados em um cluster do Cloud Volumes ONTAP, a função de administrador do Cloud Volumes ONTAP também é necessária. Saiba mais link:reference-roles.html["Funções e privilégios de backup e recuperação"]. https://docs.netapp.com/us-en/console-setup-admin/reference-iam-predefined-roles.html["Saiba mais sobre as funções de acesso do NetApp Console para todos os serviços"^].

.Passos
. No menu NetApp Backup and Recovery , selecione *Inventário*.
. Selecione uma carga de trabalho para visualizar os detalhes da proteção.
. Selecione o ícone Açõesimage:../media/icon-action.png["Opção de ações"] > *Ver detalhes*.
. Selecione a guia *Grupos de proteção*, *Grupos de disponibilidade*, *Instâncias* ou *Bancos de dados*.
...
. Selecione a política a ser aplicada ao backup.
...
. Selecione uma carga de trabalho para visualizar os detalhes da proteção.
. Selecione o ícone Açõesimage:../media/icon-action.png["Opção de ações"] > *Ver detalhes*.
. Selecione a guia *Grupos de proteção*, *Instâncias* ou *Bancos de dados*.
